...
layout = [[sg.Text('This program is used to compile and sort recorded piers and check for missing and duplicate piers.')],
...
# Create the Window
window = sg.Window('Pier Chcker v1.0', layout, default_element_size=(40, 1), grab_anywhere=False)
...
while True:
    ...
    if event == 'Run':
            ...
            dfDate = pd.read_excel(filePath, sheet_name = 0, usecols=[14], skiprows=(2))
            ...
            df = pd.read_excel(filePath, sheet_name=0, usecols=[1], skiprows=(11))
            df = df.dropna()
            dfSeries = df.squeeze()         
